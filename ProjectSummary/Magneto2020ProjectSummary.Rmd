...
The idea was to use trace identification through seperation (TIS).  This was besed off of the old find_peaks function, which was the peaks of the computed row sums. These peaks gave the computer an estimate of where the peaks may start, as that would correspond to the middle of the peak, where the width of each peak could be the spread of the individual traces; and thus, the reason that we created the improved version of the find_peaks function was to improve this algorithm.  Using the starts and ends of the first two peaks(which correspond to the top two traces), a trial and error was put in place to attempt to draw three horizontal lines.  The first, between the writing at the top of the image, and the top, first, trace. The second, between the first and second trace, these are the two traces that we really wanted.  The third being a line between the second trace and the timing marks. One of the things we tried to improve with the new algorithm was, to have these lines be closer to the actual traces, rather then be straight lines, that only change direction when they hit the trace.  After these two traces were isolated inbetween the three lines, the script would try to find the start and run down the bright trace lines, with the requirement that the script line couldn't touch the lines on the top and bottom of the image.
