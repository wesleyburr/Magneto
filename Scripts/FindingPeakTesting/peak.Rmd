...
file_loc <- as.character(ImageTesting$ImagePath[i])
...
source("~/Magneto2020/Scripts/Functions.R")
  ...
  #also need to pass in the rowsums
  peakStart <- vector()
  peakEnd <- vector()
  for (i in 1:length(fourPeaks$PeakIndex)) {
    height <- fourPeaks$PeakHeight[i]
    tempHeightLeft <- height
    tempHeightRight <- height
    
    for (j in 1:100) {
      leftSide <- fourPeaks$PeakIndex[i] - j
      rightSide <- fourPeaks$PeakIndex[i] + j
      if (rowSums[leftSide] <= tempHeightLeft & rowSums[rightSide] <= tempHeightRight) {
        tempHeightLeft <- rowSums[leftSide]
        tempHeightRight <- rowSums[rightSide]
      }
      #else if(rowSums[leftSide + 1] <= tempHeightLeft & rowSums[rightSide] <= tempHeightRight) {
       # tempHeightLeft <- rowSums[leftSide + 1]
      #  tempHeightRight <- rowSums[rightSide]
      #}
      else if(rowSums[leftSide + 1] <= tempHeightLeft & rowSums[rightSide + 1] <= tempHeightRight) {
        tempHeightLeft <- rowSums[leftSide + 1]
        tempHeightRight <- rowSums[rightSide + 1]
      }
      else{
        peakStart[i] <- leftSide
        peakEnd[i] <- rightSide
        break
      }
  ...
  abline(v = c(peakStart, peakEnd), col = 'green', lty = 2)
  abline(v = c(pracemaPeaks[,3], pracemaPeaks[,4]), lty = 2, col = "red")
