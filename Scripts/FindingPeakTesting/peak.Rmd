...
```
```{r}
rowSums <-readRDS("~/Magneto2020/rowSums.RDS")
...
file_loc <- "/home/ben/magnetoMARK/Images/Range/AGC-H-19260101-19281230/"#as.character(ImageTesting$ImagePath[i])
imageName <- "AGC--H-19260107-19260109.tif"#as.character(ImageTesting$ImageName[i])


image <- image_import(imageName, file_loc)
source("~/Magneto2020/Scripts/Functions/Functions.R")
  ...
  gaussImage <- abs(t( apply(image, MARGIN = 1, FUN = deconvGauss, sig = 10, kern.trunc = 0.05, nw = 3 ) ))
    ...
    distance <- 100
    saveRDS(rowSums, file = "~/Magneto2020/rowSums.RDS")
    fourPeaks <- find_peaks(rowSums = rowSums, minDistance =  distance, maxPeakNumber = 4, percentFromEdge = 0.9)
  }
  if(bright == TRUE) {# this is bright = TRUE
    ...
    fourPeaks <- find_peaks(rowSums,minDistance =  distance, 4, percentFromEdge = 0.9)
  ...
  plot(rowSums, type = "l")
  points(x = fourPeaks$peakIndex, y = fourPeaks$peakHeight, pch = 20, col = 'red')
  points(x = pracemaPeaks[,2], y = pracemaPeaks[,1 ], pch = 11, col = 'blue')
  abline(v = c(fourPeaks$peakStart, fourPeaks$peakEnd), col = 'black', lty = 2)
  abline(v = c(pracemaPeaks[,3], pracemaPeaks[,4]), lty = 2, col = "green")
