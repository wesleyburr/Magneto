...
source("~/Magneto2020/Scripts/Functions.R")
  ...
  if (bright == FALSE) {
    ...
    gaussImage[(nrow(gaussImage) - 125):(nrow(gaussImage)),] <- mean(gaussImage)
    gaussImage[gaussImage < 0] <- 0

    col_sums <- colSums(gaussImage)
    rowSums <- rowSums(gaussImage)^2
    threshold <- (0.8*mean(rowSums))
    fivePercent <- (sum(gaussImage[(nrow(gaussImage) - 126),]) + 1)^2#0.05*max(rowSums)
    distance <- 100
  ...
  if(bright == TRUE) {# this is bright = TRUE
    ...
    rowSums <- rowSums(gaussImage)^2
    threshold <- (0.4*(mean(rowSums)))
    fivePercent <- 0.015*max(rowSums)#(sum(gaussImage[(nrow(gaussImage) - 126),]) + 1)^2
    distance <- 50
