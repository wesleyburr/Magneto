...
source("~/Magneto2020/Scripts/Functions.R")
  ...
  #TODO need to look into puting this straight into the find peaks function! instead of making it its own
  GetPeakStartAndEnd <- function(rowSums, FourPeaksDF)
  ...
  for (k in 1:length(fourPeaks$PeakIndex)) {
    ...
    for (j in 1:100) {
      ...
      else if(rowSums[leftSide] <= tempHeightLeft & rowSums[rightSide + 1] <= tempHeightRight) {
        tempHeightLeft <- rowSums[leftSide]
  ...
  abline(v = c(peakStart, peakEnd), col = 'black', lty = 2)
  abline(v = c(pracemaPeaks[,3], pracemaPeaks[,4]), lty = 2, col = "green")
