...
```{r}
opinion1_209 = c(userOpinion1162[1:116],userOpinion117_209[117:209])
```
...
path <- vector()
for (i in 1:209){
  name[i] <- brightnessFirst209$ImageName[i]
  path[i] <- brightnessFirst209$ImagePath[i]
  image <- image_import(name[i],path[i])
  
  above.8[i] <- length(which(image >= 0.8))
...
```{r}
xvals = 1:209
Images <- data.frame(standard, opinion1_209)
plot(x = xvals, y = Images$standard, col = Images$opinion1_209)
...
```{r}
forlm <- data.frame(standard, opinion1_209)
forlm <- forlm[-192,]
lmod <- glm(opinion1_209 ~ standard, data = forlm, family = binomial)
summary(lmod)
plot(lmod, which = 1:6)
```
...
```{r}
nullmodel <- glm(opinion1_209~1, family = binomial,  data = forlm)
...
name <- "AGC--H-19260805-19260807.tif"
...
plot(WithGauss)
  ...
  abovespec <- length(which(image >= 0.75))
...
```{r}
saveRDS(opinion1_209, file = "~/Magneto2020/200624first209opinionsB")
```
